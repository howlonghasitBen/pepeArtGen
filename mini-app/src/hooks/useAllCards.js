    ...
    try {
      ...
      const cardsPromises = logs.map(async (log) => {
        try {
          ...
          console.log(`  🎴 Processing token #${tokenId} (minted by ${minter})`)
  ...
  const fetchCards = useCallback(async () => {
    ...
    try {
      ...
      if (backendCards && backendCards.length > 0) {
        ...
        formattedCards.sort((a, b) => new Date(b.mintedAt) - new Date(a.mintedAt))
      ...
      try {
        ...
        if (blockchainCards.length > 0) {
          console.log(`✅ Loaded ${blockchainCards.length} total cards from blockchain`)
