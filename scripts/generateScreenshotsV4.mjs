#!/usr/bin/env node
/**
 * V4 - Uses EXACT generateCardHTML from screenshotService.mjs
 * No custom themes - uses wavestcg defaults
 */
...
/**
 * EXACT copy from screenshotService.mjs
 */
...
async function generateFlavorText(cardName, cardType, attack, defense) {
  const prompt = `Write a short, evocative flavor text (1-2 sentences, max 100 chars) for a trading card called "${cardName}". It's a ${cardType} with ${attack} attack and ${defense} defense. Be creative, mysterious, or funny. No quotes.`;
  ...
  try {
    ...
    return data.response?.trim().replace(/^["']|["']$/g, '').slice(0, 150) || null;
  } catch (e) {
    return null;
  }
}
 ...
 * NO CUSTOM THEME - uses defaults from generateCardHTML
 */
function transformMetadata(metadata, cardId, flavorText, stats) {
  ...
  return {
    ...
    artist: attrs['Artist'] || 'Waves TCG',
    rarity: attrs['Rarity'] || '1/1',
    manaCost: manaOrbs
    // NO theme - let defaults apply
  };
}

function randomizeStats(baseAttack, baseDefense) {
  const variance = Math.floor(Math.random() * 3) - 1;
  let attack = Math.max(1, Math.min(10, baseAttack + variance));
  let defense = Math.max(1, Math.min(10, baseDefense - variance));
...
async function main() {
  ...
  console.log(`📸 V4: ${cards.length} screenshots with EXACT wavestcg HTML...\n`);
